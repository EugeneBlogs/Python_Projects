...
info_lbl = Label(
    ...
    text="Готовьте свои щёки - будет очень весело😂\n"
         "Выберите название текста (кстати все тексты авторского сочинения) из предложенного списка, "
         "затем ответьте на вопросы. Ответом может служить как одно слово, так и словосочетания или оборот "
         "(причастный и деепричастный).\nДанные ответы будут подставлены в текст. Так как вы не знали контекст, "
         "получится смешная история :)",
    font=("Ink Free", 24),
    fg="red",
    wraplength=750
)
