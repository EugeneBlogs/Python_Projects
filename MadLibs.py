...
info_lbl = Label(
    frame,
    text="Готовьте свои щёки - будет очень весело😂\nВыберите название текста (кстати все тексты сгенерированы нейросетью) из предложенного списка, затем ответьте на вопросы. Ответом может служить как одно слово, так и словосочетания или оборот (причастный и деепричастный).\nДанные ответы будут подставлены в текст. Так как вы не знали контекст, получится смешная история :)",
    font=("Ink Free", 16),
    fg="red",
    wraplength=650
)
